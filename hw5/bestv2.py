...
if __name__ == "__main__":
    ...
    for i in range(tar_labels.shape[0]):
        ...
        #img = basic_iterative(img, model, trans, inv_trans, tar_label)
        N = 3#int(min(30+4, 1.25 * 30))
        alpha = 0.8 / 255
        for j in range(N):
            ...
            loss.backward()
            update = img.grad.sign_()
            update = update.squeeze(0)
            img_d = img.data.squeeze(0)
            img_d = inv_trans(img_d)
            img_d = torch.clamp(img_d - alpha * update, min=0.0, max=1)
            img_d = trans(img_d)
            img.data= img_d.unsqueeze(0)
        ...
        #print(np.array(img, dtype=float).reshape(-1)*255)
        #print( np.array(raw_imgs[i], dtype=float).reshape(-1))
        L_inf = max(abs( np.array(img * 255, dtype=float).reshape(-1) - np.array(raw_imgs[i], dtype=float).reshape(-1)) )
        L_infs[i] = L_inf
