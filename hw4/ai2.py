...
def show_saliency_maps(x, y, model, offset):
    ...
    for i in range(num_pics):
        ...
        if REPORT_MODE:
            ...
            divider = make_axes_locatable(ax)
            #cax = divider.append_axes("right", size="5%", pad=0.05)
            plt.savefig('rep/test'+str(i+offset)+'.jpg')
            #plt.show()
            plt.close()
        else:
            ax =  plt.gca()
...
if __name__ == "__main__":
    ...
    for i in range(0, len(imgs), 10):
        end = (i+10) if (i+10) < len(imgs) else len(imgs)
        show_saliency_maps(imgs_tensor[i:end], label_tensor[i:end], model, i)  
    #ll = [5,18,23,30,41,52,60]
    #lime_imgs = np.concatenate((imgs[5:6],imgs[18:19],imgs[23:24],imgs[30:31]\
    #    ,imgs[41:42],imgs[52:53],imgs[60:61]), axis=0)
    #lime_imgs_tensor = torch.tensor(lime_imgs).type(torch.FloatTensor)
    #lime_label_tensor = torch.tensor(np.arange(7)).type(torch.LongTensor)
    #show_saliency_maps(lime_imgs_tensor, lime_label_tensor, model)    
